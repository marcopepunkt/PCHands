...
def build_free_root(robot_builder, robot_name="", translation_range=(-1, 1),
    ...
    for i in range(6):
        ...
        if i == 3 or i == 0:
            child.set_joint_properties(joint_types[i], limits=np.array([joint_limit[i]]),
                                       pose_in_parent=Pose(), pose_in_child=Pose())
        elif i == 4 or i == 1:
            child.set_joint_properties(joint_types[i], limits=np.array([joint_limit[i]]),
...
def build_ball_joint(robot_builder, parent, name, pose):
...
def fix_link_inertia(robot_builder):
    for builder in robot_builder.link_builders:
        builder.set_mass_and_inertia(1e-2, Pose(np.zeros(3)), np.ones(3) * 1e-4)
        # if len(builder.get_collisions()) < 1 and 'A_' not in builder.get_name():
        #     builder.set_mass_and_inertia(1e-2, Pose(np.zeros(3)), np.ones(3) * 1e-4)


def create_visual_material(specular, metallic, roughness, base_color):
